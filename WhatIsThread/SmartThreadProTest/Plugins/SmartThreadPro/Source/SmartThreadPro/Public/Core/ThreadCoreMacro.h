...
DEFINE_LOG_CATEGORY_STATIC(SmartThreadProLOG, Log, All);
...
{FAsyncTask<FSmartAbandonable> *SmartAbandonable = new FAsyncTask<FSmartAbandonable>(ThreadDelegate); \
SmartAbandonable->StartBackgroundTask(); \
SmartAbandonable->EnsureCompletion(); \
delete SmartAbandonable; }
...
#define SMART_THREAD_PRO_INFO_MSG(Format, ...) \
{ \
	SET_WARN_COLOR(COLOR_CYAN); \
	const FString Msg = FString::Printf(TEXT(Format), ##__VA_ARGS__); \
	UE_LOG(SmartThreadProLOG, Log, TEXT("%s"), *Msg); \
	CLEAR_WARN_COLOR(); \
}

#define SMART_THREAD_PRO_INFO_MSG_WARNING(Format, ...) \
{ \
	SET_WARN_COLOR(COLOR_CYAN); \
	const FString Msg = FString::Printf(TEXT(Format), ##__VA_ARGS__); \
	UE_LOG(SmartThreadProLOG, Error, TEXT("%s"), *Msg); \
	CLEAR_WARN_COLOR(); \
}

#define SMART_THREAD_PRO_INFO_MSG_ERROR(Format, ...) \
{ \
	SET_WARN_COLOR(COLOR_CYAN); \
	const FString Msg = FString::Printf(TEXT(Format), ##__VA_ARGS__); \
	UE_LOG(SmartThreadProLOG, Error, TEXT("%s"), *Msg); \
